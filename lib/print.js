...
export default async function (m, conn = { user: {} }) {
  if (!m) return
  const db = global.db?.data || global.DATABASE?.data || { users: {} }
  const senderJid = m.sender || m.key?.participant || m.key?.remoteJid || ''
  const chatJid = m.chat || m.key?.remoteJid || ''

  let _name = ''
  try {
    _name = senderJid && typeof conn.getName === 'function' ? await conn.getName(senderJid) : ''
  } catch {
    _name = ''
  }

  let senderNumber = senderJid.replace(/[^0-9]/g, '')
  let senderIntl = senderNumber
    ? PhoneNumber('+' + senderNumber).getNumber('international')
    : senderJid || 'unknown'
  let sender = senderIntl + (_name ? ' ~' + _name : '')

  let chat = ''
  try {
    chat = chatJid && typeof conn.getName === 'function' ? await conn.getName(chatJid) : ''
  } catch {
    chat = chatJid || ''
  }
  // let ansi = '\x1b['
  ...
  let user = db.users?.[senderJid] || {}
  const botJid = conn.user?.jid || conn.user?.id || ''
  const botNumber = botJid.replace(/[^0-9]/g, '')
  let me = botNumber ? PhoneNumber('+' + botNumber).getNumber('international') : botJid || 'bot'
...
`.trim(), 
    me +
      ...
      `${(conn.user?.jid || conn.user?.id || '') == (global.conn?.user?.jid || global.conn?.user?.id || '') ? '' : ' (Sub Bot)'}`,
